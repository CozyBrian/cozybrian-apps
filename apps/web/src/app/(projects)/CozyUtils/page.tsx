...
export const metadata: Metadata = {
  title: "CozyUtils",
  description:
    "A small Rust CLI for asset automation and AI-assisted Git message workflows.",
  keywords: [
    "Brian Newton",
    "CozyBrian",
    "CozyUtils",
        ...
        <section className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <p className="text-xs uppercase tracking-[0.2em] text-cozy-500">
              Small, cross-platform Rust CLI for builders.
            </p>
          ...
          <p className="text-sm md:text-base text-cozy-300 leading-7 max-w-[680px]">
            CozyUtils streamlines repetitive project setup and Git workflows. It
            converts assets, generates export indexes, and creates commit and PR
            messages through configurable AI providers.
          </p>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            },
            {
              ...
              description:
                "Generate commit and PR messages from Git diffs with clipboard, file output, and commit support.",
            },
            {
              title: "Flexible AI Providers",
              description:
                "Use built-in Gemini, OpenRouter, DeepSeek, or OpenCode providers, or configure any OpenAI-compatible API.",
            },
            {
...
./cozyutils -prmsg --base=origin/dev --provider=openrouter --clipboard`}</code>
            </pre>
          </div>
        </section>
        ...
        <section className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              ...
              "-cmsg [--out=path] [--model=MODEL] [--provider=NAME] [--clipboard] [--commit]",
              "-prmsg [--base=origin/dev] [--out=path] [--model=MODEL] [--provider=NAME] [--clipboard]",
              "-config [--init] [--show] [--set-default-provider=NAME]",
            ].map((command) => (
              <div
                key={command}
        ...
        <section className="flex flex-col gap-3">
          ...
          <p className="text-sm text-cozy-400 leading-6 max-w-[680px]">
            Configure a default provider, command-specific providers, models,
            and API key environment variables in `~/.cozyutils/config.json`.
            CozyUtils includes templates for Gemini, OpenRouter, DeepSeek, and
            OpenCode, and supports custom OpenAI-compatible providers. Create a
            starter config with:
          </p>
          <div className="bg-cozy-950 border border-cozy-900 p-4">
            <pre className="text-xs text-cozy-200 leading-6 overflow-x-auto">
              <code>{`./cozyutils -config --init
./cozyutils -config --set-default-provider=openrouter
./cozyutils -config --show`}</code>
            </pre>
          </div>
        </section>
      </main>
